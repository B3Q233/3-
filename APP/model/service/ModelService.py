...
def get_model_by_id(model_id):
    get_model = ModelsModel.find_model_by_id(model_id)
...
def whole_model_update(new_model):
    found_model = ModelsModel.find_model_by_name(new_model.model_name)
    if found_model is not None:
        if found_model.model_name != new_model.model_name:
            return '模型名重复，请重新填写', False
    status = ModelsModel.update_whole_model(new_model)
    if not status:
        return '模型更新失败', False
    else:
        return '模型更新成功', True
